...
const languages = [
  { code: "en", labelKey: "profile.language_option_en" },
  { code: "es", labelKey: "profile.language_option_es" },
  { code: "fi", labelKey: "profile.language_option_fi" },
  { code: "sv", labelKey: "profile.language_option_sv" },
];
...
export const OnboardingFlow: React.FC<OnboardingFlowProps> = ({ onComplete }) => {
  ...
  const handleNext = () => {
    if (step === 0 && !hasSelectedLanguage) return;
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-background-dark flex flex-col overflow-hidden">
      <div className="flex justify-center gap-2 pt-12 pb-6">
        {[0, 1, 2, 3].map((index) => (
          <div key={index} className={`w-2 h-2 rounded-full transition-all duration-300 ${step === index ? "w-6 bg-primary" : "bg-white/20"}`} />
        ))}
      </div>
          ...
          {step === 0 && (
            ...
            >
              <div className="w-20 h-20 rounded-2xl bg-primary/20 flex items-center justify-center mb-8 overflow-hidden shadow-lg shadow-primary/20">
                <img src="/assets/logo.jpg" alt={t("chat.sender_mima")} className="w-16 h-16 object-cover rounded-xl" />
              </div>
              <h2 className="text-2xl font-bold text-white mb-2 text-center">{t("onboarding.language_title")}</h2>
              <p className="text-slate-400 text-center mb-8">{t("onboarding.language_subtitle")}</p>
          ...
          {step === 1 && (
            ...
            >
              <div className="w-32 h-32 rounded-3xl bg-primary/20 flex items-center justify-center mb-8 animate-pulse">
                <img src="https://i.postimg.cc/cJwnS5cZ/mima_logo.jpg" alt={t("chat.sender_mima")} className="w-24 h-24 rounded-2xl object-cover" />
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center ${isSelected ? "bg-primary text-white" : "bg-white/10 text-slate-400"}`}>
          ...
          {step === 3 && (
            ...
            >
              <div className="w-24 h-24 rounded-full bg-primary/20 flex items-center justify-center mb-8 overflow-hidden shadow-lg shadow-primary/30">
                <img src="/assets/logo.jpg" alt={t("chat.sender_mima")} className="w-20 h-20 object-cover rounded-full" />
              </div>
      ...
      <div className="p-8 flex gap-4 h-28">
        {step > 0 ? (
          <button onClick={handleBack} className="flex-1 py-4 rounded-2xl bg-white/5 text-white font-bold flex items-center justify-center gap-2 hover:bg-white/10 transition-colors">
        ...
        <button
          onClick={handleNext}
          disabled={step === 0 && !hasSelectedLanguage}
          className="flex-[2] py-4 rounded-2xl bg-primary text-white font-bold flex items-center justify-center gap-2 hover:bg-primary-dark transition-all active:scale-95 shadow-lg shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed"
        >
